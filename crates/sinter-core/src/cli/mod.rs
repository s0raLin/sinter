...
pub enum Commands {
    ...
    /// 插件命令 — 未被识别为内置命令, 由 executor 通过 plugins 列表处理
...
impl Cli {
    ...
    /// 测试友好的解析 — 用 try_get_matches_from 避免 process::exit
    pub fn parse_from_args(args: &[&str]) -> Self {
        let cmd = build_cli_command(&[]);
        let matches = cmd.try_get_matches_from(args).unwrap_or_default();
    ...
    pub fn parse_with_plugins(plugins: &[Box<dyn crate::core::CommandHandler>]) -> Self {
        let cmd = build_cli_command(plugins);
...
fn build_cli_command(plugins: &[Box<dyn crate::core::CommandHandler>]) -> Command {
    let mut cmd = Command::new("sinter")
        ...
        .subcommand(Command::new("new").about("创建一个新的 Scala 项目")
            ...
            .arg(Arg::new("backend").long("backend").short('b')
                .help("构建后端: scala-cli, sbt, maven, gradle").value_name("BACKEND")))
