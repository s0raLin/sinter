...
pub async fn setup_bsp(
    bsp_dir: &Path,
    deps: &[Dependency],
    source_dirs: &[(String, String)],
    backend: &str,
) -> anyhow::Result<()> {
    eprintln!(
        "DEBUG: setup_bsp called with bsp_dir: {}",
        bsp_dir.display()
    );
    ...
    match backend {
        "scala-cli" => {
            // Get scala-cli path
            let scala_cli_path = crate::build::get_scala_cli_path()
                .await
                .ok_or_else(|| anyhow::anyhow!("scala-cli is not available"))?;

            // Get scala-cli version
            let scala_cli_version = get_scala_cli_version(&scala_cli_path)
                .await
            ...
            // Get absolute paths
            let scala_build_path_abs = scala_build_path
                .canonicalize()
            ...
            write(
                &PathWrapper::new(&scala_cli_json_path),
                &scala_cli_json_content,
            )
            .await?;
            ...
            let scalac_options: Vec<String> = source_dirs
                .iter()
                .map(|(member_name, source_dir)| {
                ...
                .collect();
            let template_path =
                crate::toolkit::path::paths::template_file("ide-options-v2.json.template");
            ...
            eprintln!(
                "DEBUG: Successfully created .bsp and .scala-build directories in {}",
                bsp_dir.display()
            );
        }
        "sbt" | "gradle" | "maven" => {
